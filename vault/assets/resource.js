...
(function () {
  ...
  /* ---------- hand the workflow to Claude ----------
     Copies the whole workflow with a preface that tells Claude to run it. */
  var HAND_PREFACE = [
    'Below is a workflow from The Vault. Run it for me.',
    '',
    'Read all of it first. Then:',
    '1. Tell me in three lines what you are about to produce.',
    '2. Ask me, in one batch, only the things you cannot work out on your own.',
    '3. Produce every prompt, file, script and setting the workflow calls for, filled in for my case and ready to paste into the tools it names.',
    '',
    'Follow the workflow exactly where it is specific and use your own judgement where it leaves room. If something in it cannot be done as written, say so instead of inventing a way around it. Do not invent numbers, results or claims about my product.',
    '',
    '--- THE WORKFLOW ---',
    ''
  ].join('\n');

  function workflowText(btn) {
    var prose = btn.closest('.prose');
    if (!prose) return '';
    var parts = [];
    Array.prototype.forEach.call(prose.children, function (el) {
      if (el.classList.contains('hand')) return;
      var t = el.innerText;
      if (t && t.trim()) parts.push(t.trim());
    });
    // Drop the interface: button captions sit inside the blocks we are reading.
    var txt = parts.join('\n\n').replace(/ /g, ' ');
    txt = txt.split('\n').filter(function (l) {
      return !/^\s*(copy|copied|reset|press ctrl\+c|\d+ of \d+ checked)\s*$/i.test(l);
    }).join('\n');
    return HAND_PREFACE + txt;
  }

  document.addEventListener('click', function (e) {
    var b = e.target.closest('.hand-copy'); if (!b) return;
    var label = b.getAttribute('data-label') || b.textContent;
    b.setAttribute('data-label', label);
    copyText(workflowText(b)).then(function () {
      b.classList.add('done'); b.textContent = 'Copied. Paste it into Claude';
      setTimeout(function () { b.classList.remove('done'); b.textContent = label; }, 2600);
    }, function () {
      b.textContent = 'Could not copy. Select the page and press Ctrl+C';
      setTimeout(function () { b.textContent = label; }, 3200);
    });
  });
  ...
  function loadContent() {
    ...
    window.Vault.api('/api/content/' + encodeURIComponent(slug)).then(function (d) {
      ...
      buildToc();
      initChecks(root);
      if (window.Vault.reveal) window.Vault.reveal(root);
    });
  }
