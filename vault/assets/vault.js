...
(function () {
  ...
  var CONFIG = {
    ...
    plans: {
      ...
      monthly: {
        ...
        billed: '$10.99', billedNote: 'billed every month', note: 'Cancel any time'
      }
    }
  };
  ...
  /* ---------- reveal on scroll ----------
     Blocks start at opacity 0 and fade in. A block that is never told to
     appear is a screen of nothing, so this is deliberately forgiving:
     threshold 0 (any pixel on screen counts, however tall the block),
     anything already scrolled past is shown at once, and a timer sweeps up
     whatever the observer missed. Call revealIn(node) after injecting HTML. */
  var io = null;
  ...
  function revealIn(scope) {
    var list = (scope || document).querySelectorAll('.rv:not(.in)');
    Array.prototype.forEach.call(list, function (el) {
      var r = el.getBoundingClientRect();
      if (!io || r.top < window.innerHeight + 80) { el.classList.add('in'); return; }
      io.observe(el);
    });
    if (io) setTimeout(function () { sweep(scope); }, 2500);
  }
  ...
  revealIn(document);
  window.addEventListener('scroll', function () { sweep(document); }, { passive: true });
  ...
  window.Vault = {
    ...
    portal: portal, toast: toast, esc: esc, reveal: revealIn
  };
})();
